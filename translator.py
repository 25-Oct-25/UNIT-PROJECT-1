...
class TranslatorUser(CommunityUser):
    ...
    def __init__(self, name ,location ,email ,translation_major ,translation_type ,registration_date , gender:str, language:str,  official_certificate:bool,  years_of_experience:int=0 ,rate:int = 0, more_language_list=[]):
    ...
    def user_information(self):
        return f"Translator name: {self.name}, Location: {self.location}, Gender: {self.gender}, Major: {self.translation_major}, Mother language: {self.language}, and other languages: {self.show_language_list()}, Offical certificates: {self.official_certificate}, Years of exoerience: {self.years_of_experience}, Rate: {self.rate}"
